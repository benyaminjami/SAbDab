...
from data.configs import AG_TYPES, RAbD_PDB, IGFOLD_TEST_PDB, SKEMPI_PDB
from data.configs import IMGT
from .pdb_utils import Protein, AgAbComplex, Peptide
from utils.io import read_csv
from utils.network import url_get
from utils.logger import print_log
from data.split import load_file
...
def fetch_from_pdb(identifier, tries=5):
    ...
    if text is None:
        return None
        print_log(f"Trying CIF version for {identifier}", level="WARN")

        # url = f"https://files.rcsb.org/download/{identifier}.cif"

        # cif = url_get(url, tries)
        # if text is None:
        #     return None
...
def download_one_item_local(pdb_dir, item):
    ...
    if pdb_id in ["3h3b", "2ghw", "3uzq"]:
        ...
        with open(
            os.path.join(file_dir, "..", "summaries", pdb_id + ".pdb"), "r"
        ) as fin:
...
def post_process(item):
    ...
    # if (
    #     item["light_chain"] == "" or item["heavy_chain"] == ""
    # ):  # we do not handle nanobodies
    ...
    if numbering == "none":
        ...
        if not item["pre_numbered"]:
            ...
            if exit_code != 0:
                print_log(
                    f"prenumbering failed for {pdb}. scheme {numbering}", level="ERROR"
                )
                return cleanup_none(pdb_data_path)
        try:
            # clean pdb
            if (
                item["heavy_chain"].lower() == item["light_chain"].lower()
            ):  # H/L is connected
                ab_chain = item["heavy_chain"].upper()
                prot = Protein.from_pdb(pdb_data_path)
                if prot.get_chain(item["heavy_chain"]) is None or prot.get_chain(item["light_chain"]) is None:
                    ...
                    for i, residue in enumerate(ab_chain_pep):
                        if residue.get_id()[0] > IMGT.HFR4[-1]:
                            ...
                            break
                    hc, lc = (
                        ab_chain_pep.residues[:light_chain_start],
                        ab_chain_pep.residues[light_chain_start:],
                    )
                    ...
                    if exit_code != 0:
                        print_log(
                            f"renumbering failed for {pdb}. scheme {numbering}",
                            level="ERROR",
                        )
                        return cleanup_none(pdb_data_path)
            ...
            cplx = AgAbComplex.from_pdb(
                pdb_data_path, H, L, A, numbering=numbering, skip_epitope_cal=False
            )
            cplx.to_pdb(pdb_data_path)

            cplx.antigen.to_pdb(pdb_data_path[:-4]+'_ag.pdb')
            cplx.antibody.to_pdb(pdb_data_path[:-4]+'_ab.pdb')
            ...
            print_log(
                f"parsing pdb failed for {pdb} unexpectedly. reason: {e}", level="WARN"
            )
            return cleanup_none(pdb_data_path)
        ...
        if item['light_chain'] != "":
            ...
            item["light_chain_seq"] = ""
        for c in ["H", "L"]:
            if c == 'L' and item['light_chain'] == "":
                continue
            for i in range(1, 4):
        ...
        item['epitop_dists'] = cplx._cal_linear_epitope()
        epitope = cplx.get_epitope()
        paratope_dist = []
        chain_list = [cplx.get_heavy_chain()]
        if item['light_chain'] != "":
            chain_list = [cplx.get_light_chain(), cplx.get_heavy_chain()]
        for chain in chain_list:
            chain_dist = []
            for ab_residue in chain:
                m = 1e10
                for ag_residue, _, _ in epitope:
                    dist = int(ab_residue.dist_to(ag_residue)*100)/100
                    m = min(dist, m)
                chain_dist.append(m)
            paratope_dist.append(chain_dist)
        item['paratope_dist'] = paratope_dist
        if item['light_chain'] != "":
            item['position_ids'] = [
                [str(r.id[0])+r.id[1].replace(' ', '') for r in cplx.antibody.peptides[L].residues],
                [str(r.id[0])+r.id[1].replace(' ', '') for r in cplx.antibody.peptides[H].residues]
            ]
        else:
            item['position_ids'] = [
                [str(r.id[0])+r.id[1].replace(' ', '') for r in cplx.antibody.peptides[H].residues]
...
def download(
    items, out_path, ncpu=8, pdb_dir=None, numbering="imgt", pre_numbered=False, skip_download=False
):
    if pdb_dir is None:
    ...
    if not skip_download:
        ...
        for item in tqdm(valid_entries):
            ...
            version = 1
            root, ext = os.path.splitext(pdb_fout)
            while True:
                if not os.path.exists(pdb_fout):
                    break
                pdb_fout = f"{root}_{version}{ext}"
                if not os.path.exists(pdb_fout):
                    break
                version += 1
        ...
        print_log(f"number of temp-valid entries: {len(valid_entries)}")
    ...
    valid_entries = load_file(out_path)

    print_log("post processing: data cleaning")
    # t = [post_process(ent) for ent in valid_entries]
    valid_entries = process_map(
        post_process, valid_entries, max_workers=ncpu, chunksize=1
    )
    valid_entries = [item for item in valid_entries if item is not None]
...
def parse():
    ...
    parser.add_argument(
        "--summary", type=str, required=True, help="Path to summary file"
    )
    parser.add_argument(
        "--fout", type=str, required=True, help="Path to output json file"
    )
    parser.add_argument(
        "--type",
        type=str,
        choices=["skempi", "sabdab", "rabd", "igfold_test"],
        default="sabdab",
        help="Type of the dataset",
    )
    parser.add_argument(
        "--pdb_dir", type=str, default=None, help="Path to local folder of PDB files"
    )
    parser.add_argument(
        "--numbering",
        type=str,
    ...
    parser.add_argument(
        "--pre_numbered",
        action="store_true",
        help="The files in pdb_dir is already renumbered",
    )
...
def main(args):
    ...
    if args.pdb_dir is not None:
        ...
        if args.pre_numbered:
            print_log(
                f"Assume PDB file already renumbered with scheme {args.numbering}"
            )
    items = download(
        items, out_path, args.n_cpu, args.pdb_dir, args.numbering, args.pre_numbered
    )
...
if __name__ == "__main__":
    ...
    #         "light_chain": "E",
    #         "antigen_chains": ["X", "R"],
    #         "pdb_data_path": "/scratch/ssd004/scratch/benjami/dyMEAN/all_data/pdb/8gpi (1).pdb",
    #         "numbering": "imgt",
    #         "pre_numbered": False,
    #     }
    # )
