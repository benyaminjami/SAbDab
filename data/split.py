...
def main(args):
    ...
    if args.benchmark is not None:
        ...
        nano = np.array(filter_flag(items, '101'))
        paired_clusters = [c for c in other_clusters if nano[clu2idx[c]].sum() == 0]
        np.random.shuffle(paired_clusters)
        valid_len = int(len(paired_clusters) * args.valid_ratio)
        valid_clusters = paired_clusters[-valid_len:]
        train_clusters = [c for c in other_clusters if c not in valid_clusters]
