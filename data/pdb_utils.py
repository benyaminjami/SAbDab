...
import re
from data.configs import IMGT, Chothia
...
class Residue:
    ...
    def get_backbone_coord_map(self) -> Dict[str, List]:
        coord = {
            atom: self.coordinate[atom]
            for atom in self.coordinate
            if atom in VOCAB.backbone_atoms
        }
        return coord
    ...
    def set_symbol(self, symbol):
        assert (
            VOCAB.symbol_to_abrv(symbol) is not None
        ), f"{symbol} is not an amino acid"
        self.symbol = symbol
    ...
    def __iter__(self):
        return iter(
            [(atom_name, self.coordinate[atom_name]) for atom_name in self.coordinate]
        )
...
class Protein:
    ...
    def __eq__(self, other):
        ...
        for key in self.peptides:
            if (
                key in other.peptides
                and self.peptides[key].seq == other.peptides[key].seq
            ):
                continue
            else:
                return False
...
class AgAbComplex:
    ...
    def __init__(
        ...
        if skip_validity_check:
            ...
            self.antibody, self.cdr_pos = self._extract_antibody_info(
                antibody, numbering
            )
    ...
    def from_pdb(
        ...
        ag_peptides = {
            chain: protein.get_chain(chain)
            for chain in antigen_chains
            if protein.get_chain(chain) is not None
        }
        for chain in antigen_chains:
    ...
    def _extract_antibody_info(self, antibody: Protein, numbering: str):
        ...
        h_type_mapping, l_type_mapping = (
            {},
            {},
        )  # - for non-Fv region, 0 for framework, 1/2/3 for cdr1/2/3
        ...
        for c, chain_name in zip(["H", "L"], [self.heavy_chain, self.light_chain]):
            ...
            for cdr in ["1", "2", "3"]:
                ...
                # assert (
                #     cdr_start != -1
                # ), f"cdr {c}{cdr} not found, residue type: {res_type}"
                if cdr_start == -1:
                    print(f"{self.antigen.get_id()} cdr {c}{cdr} not found, residue type: {res_type}")
    ...
    def _cal_epitope(self):
        ...
        for _type, protein in zip(["ag", "ab"], [self.antigen, [("A", cdrh3)]]):
            ...
            for chain_name, chain in protein:
                for i, residue in enumerate(chain):
                    ...
                    x = [coord[key] for key in coord] + [
                        [0, 0, 0] for _ in range(num_pad)
                    ]
        ...
        # calculate distance
        ag_xs, ab_xs = np.array(ag_xs), np.array(
            ab_xs
        )  # [Nag/ab, M, 3], M == MAX_ATOM_NUM
        ag_mask, ab_mask = np.array(ag_mask).astype("bool"), np.array(ab_mask).astype(
            "bool"
        )  # [Nag/ab, M]
        dist = np.linalg.norm(ag_xs[:, None] - ab_xs[None, :], axis=-1)  # [Nag, Nab, M]
        dist = (
            dist + np.logical_not(ag_mask[:, None] * ab_mask[None, :]) * 1e6
        )  # [Nag, Nab, M]
    ...
    def _cal_linear_epitope(self, epitope_dist=8, pad=5):
        def get_clusters(lst, n_faslse=5, n_true=3):
            clusters = []
            start_idx = None
            false_count = 0  # Counter to keep track of consecutive False values
            true_count = 0
            for i, val in enumerate(lst):
                if val:
                    if start_idx is None:  # Start of a new cluster
                        start_idx = i
                    true_count += 1
                    false_count = 0  # Reset consecutive False counter
                elif not val and start_idx is not None:
                    false_count += 1
                    if (
                        false_count > n_faslse
                    ):  # If more than n_fasle consecutive Falses, end the cluster
                        if true_count >= n_true:
                            clusters.append(
                                (start_idx, i - false_count, true_count)
                            )  # Capture start and end indices
                        true_count = 0
                        start_idx = None

            # Handle case where last cluster goes till the end
            if start_idx is not None:
                end_idx = (
                    len(lst) - 1 if false_count <= 3 else start_idx + false_count - 4
                )
                clusters.append((start_idx, end_idx, true_count))
        ...
        # Iterate over the ag_rids and min_dists and populate the list
        for t, s in zip(ag_rids, min_dists):
            chain_id = t[0]
            if chain_id != current_chain_id:
                if current_scores:
                    dists_list.append(current_scores)
                current_scores = []
                current_chain_id = chain_id
            current_scores.append(round(s,1))

        # Append the last set of scores
        if current_scores:
            dists_list.append(current_scores)

        return dists_list
        # epitope_ind = np.where(min_dists <= epitope_dist)[0]
        # chains = {}
        # for idx in epitope_ind:
        #     chain_name, i = ag_rids[idx]
        #     chains[chain_name] = chains.get(chain_name, []) + [i]
        # linear_epitops = []
        # for ch in chains:
        #     epitope_ind = np.array(chains[ch])
        #     chain_seq = self.antigen.get_chain(ch).seq
        #     epitope_residues = np.array([False for _ in chain_seq])
        #     epitope_residues[epitope_ind] = True
        #     if len(chain_seq) <= 15:
        #         if len(epitope_ind) >= 3:
        #             try:
        #                 antigen = fetch_from_fasta(self.pdb_id, chain=ch)
        #             except:
        #                 antigen = chain_seq
        #             linear_epitops.append((antigen, len(epitope_ind)))
        #         continue
        #     clusters = get_clusters(epitope_residues)
        #     for c in clusters:
        #         if c[1] - c[0] > 5:
        #             start, end, n_true = c
        #             start, end = max(0, start - pad), min(len(chain_seq) - 1, end + pad)
        #             linear_epitops.append((chain_seq[start:end], n_true, ch))
        #     # start, end = epitope_range.min(), epitope_range.max()
        #     # start, end = max(0, start-pad), min(len(chain_seq)-1, end)
        #     # linear_epitops.append(chain_seq[start:end])

        # return linear_epitops
    ...
    def __str__(self):
        ...
        antibody_info = (
            f"Antibody H-{self.heavy_chain} ({len(self.get_heavy_chain())}), "
...
def fetch_from_fasta(identifier, chain):
    identifier = identifier[:4]
    url = "https://www.rcsb.org/fasta/entry/" + identifier
    text = url_get(url, tries=5)
    if text.status_code != 200:
        print(f"Bega {identifier} {chain}")
        raise NotImplementedError(f"Bega {identifier} {chain}")
    fasta_io = StringIO(text.text)
    records = SeqIO.parse(fasta_io, "fasta")
    for c in list(records):
        des = c.description.split("|")[1]
        entry_chains = re.findall(r'\b([A-Z])\b(?!\[auth)', des)
        if chain in entry_chains:
            return str(c.seq)
    print(f"Bega2 {identifier} {chain}")
    raise NotImplementedError(f"Bega2 {identifier} {chain}")
...
def cif_to_pdb(cif_path):
    par = MMCIFParser()
    struc = par.get_structure("anonym", cif_path)
    io = PDBIO()
...
if __name__ == "__main__":
    ...
    cplx = AgAbComplex.from_pdb(
        "/scratch/ssd004/scratch/benjami/dyMEAN/all_data/pdb/1a2y.pdb",
        "B",
        "A",
        [],
        numbering="imgt", skip_epitope_cal=True
    )
    print(cplx._cal_linear_epitope())
    print(cplx.get_epitope())
    print(cplx)
